...
function form_element($label, $type='text', $name, $value = '', $extra = array()) {
    ...
    if(!empty($label)) {
        $return .= '<label class="col-sm-2 control-label no-padding-right" for="'.$id.'">'.$label.'</label>';
    }
    switch ($type) {
        case 'text':
        case 'password':
        case 'number':
            ...
            $return .= '<input type="'.$type.'" id="'.$id.'" '.$extra_params.' name="'.$name.'" class="col-xs-10 col-sm-5 '.$element_classes.'" value="'.$value.'" />';
            $return .= '</div>';
            break;
        case 'switchbutton':
            $extra_params = '';
            ...
            $return .= '<div class="col-xs-3">
                            <label class="switch-radio">
                                <input id="'.$id.'" name="'.$name.'" '.$extra_params.' class="ace ace-switch ace-switch-6" type="checkbox" />
                                <span class="lbl"></span>
                            </label>
                        </div>';
            $return .= '</div>';
        ...
        case 'select':
            ...
            $return .= '<div class="col-sm-5">
                            <select class="chosen-select" '.$extra['attributes'].' id="'.$id.'" name="'.$name.'">';
        ...
        case 'textarea':
            ...
            $return .= '<div class="col-sm-5">';
            $return .= '<textarea id="'.$id.'" name="'.$name.'" class="'.$extra_class.'" '.$rows.' '.$cols.'>'.$value.'</textarea>';
            $return .= '</div>';
            break;
        case 'file':
        ...
        case 'datepicker':
            ...
            $return .= '<input class="form-control date-picker '.$element_classes.'" id="'.$id.'" name="'.$name.'" type="text" data-date-format="'.SITE_DATE_FORMAT.'" value="'.$value.'" />
